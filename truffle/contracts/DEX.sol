...
contract Dex is Ownable {
    ...
    event TokenWithdrawn(address _to, address _tokenAddress, uint _amount, uint _timeStamp);
    ...
    function depositToken(address _tokenAddress, uint _amount) public {
        ...
        addressTokenBalance[msg.sender][tokenSymbolIndex] = addressTokenBalance[msg.sender][tokenSymbolIndex].add(_amount);
    ...
    function withdrawToken(address _tokenAddress, uint _amount) public {
        ...
        uint tempVal = addressTokenBalance[msg.sender][tokenSymbolIndex].sub(_amount);
        require(tempVal >= 0,"The address token balance cannot go below zero.");

        addressTokenBalance[msg.sender][tokenSymbolIndex] = tempVal;
        require(thisToken.transfer(msg.sender, _amount) == true,"Token Withdrawal Failed");
        emit TokenWithdrawn(msg.sender,_tokenAddress,_amount,now);
    }
